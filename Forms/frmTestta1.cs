...
namespace BMS
{
    public partial class frmTestta1 : Form
    {
        ...
        private void grvData_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (grvData.Columns[e.ColumnIndex].DataPropertyName == "Result1")
            {
                grvData.CurrentCell = grvData[1, e.RowIndex + 1];
                grvData.BeginEdit(true);
            }
            else
            {
        ...
        private void grvData_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (grvData.SelectedColumns[0].DataPropertyName.Contains("Result"))
                {
                    grvData.CurrentCell = grvData[1, grvData.SelectedRows[0].Index + 1];
                    //grvData.BeginEdit(true);
                }
                else
                {
